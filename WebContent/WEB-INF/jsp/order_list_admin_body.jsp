...
<fmt:message bundle="${loc}" key="message.delivered" var="deliveredLoc" />
<fmt:message bundle="${loc}" key="message.executed" var="executeLoc" />

<c:forEach var="order" items="${orders}">
						<tr>
							<td><a class="text-dark " href="controller?command=ORDER_DETAILES&orderId=${order.id}"><c:out value="${order.id}"></c:out></a></td>
							...
							<td>
								<c:choose>
									...
									<c:when test="${order.status.name == 'DELIVERED'}">
										<c:out value="${deliveredLoc}" /></a>
									</c:when>
									<c:when test="${order.status.name == 'EXECUTED'}">
										<p class="text-success"><c:out value="${executeLoc}" /></p>
									</c:when>
								</c:choose>
							</td>
