...
//возвращает последний id in order
	...
	public int addOrder(int idUser, StatusOrder status) throws DAOException {
		...
		try {
			connection = connectionPool.getConnection();
			JDBCUtil.insertDeleteUpdate(connection, SQLQuery.ADD_ORDER, idUser, new Date(System.currentTimeMillis()), status.toString());
	...
	public List<Order> getListOrders(int limit, int offset) throws DAOException {
		...
		try {
			...
			return JDBCUtil.select(connection, SQLQuery.LIST_ORDER, resultSetHandlerOrders, limit,
					offset);
	...
	public int countOrders() throws DAOException {
		...
		try {
			...
			return JDBCUtil.select(connection, SQLQuery.COUNT_ORDERS, getterCountResultSetHandler);
